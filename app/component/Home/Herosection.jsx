...
const Hero = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
       const response = await fetch("http://127.0.0.1:8000/api/home/herosection/");
        ...
        <div className="max-w-5xl mx-auto text-center">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 dmsans">
              {result?.subtitle || "Scale Your Brand"}
            </span>
          </motion.div>
          ...
          >
            {result?.main_title || "Scale Your Brand"} <br />
            <span className="text-[#00A74E]">{result?.title_1 || "Scale Your Brand"}</span>
          </motion.h1>
          ...
          >
            <a 
              href={result?.button_link || HERO_CONTENT.cta} 
            ...
            >
              {result?.button_text || HERO_CONTENT.cta}
