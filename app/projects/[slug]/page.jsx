...
export default function ProjectDetail({ params }) {
    ...
    async function fetchProject() {
      try {
        const res = await fetch(
          `http://127.0.0.1:8000/api/core/project-posts/${slug}/`,
          { cache: "no-store" }
        );
  ...
  if (error || !project) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0A0A0A] text-white">
        <h2 className="text-3xl font-bold">{error || "Project not found"}</h2>
      </div>
    );
  }
  ...
  return (
    <section className=" bg-[#0A0A0A] text-white">
      {/* Banner */}
      <Bannermain backgroundImg={project.banner_img} title={project.title} />
      ...
      <div className="wrapper px-6 py-20 max-w-5xl mx-auto space-y-10">
        ...
        {/* Swiper Slider - Corrected for gallery_images structure */}
        {project.gallery_images && project.gallery_images.length > 0 && (
          <Swiper
            ...
            className="rounded-2xl overflow-hidden"
          >
            {project.gallery_images.map((item) => (
